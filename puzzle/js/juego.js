...
var instrucciones = [
  'Mira la imagen de la izquierda para que puedas armar la imagen de la derecha.',
  'Usa las teclas para mover las piezas del rompecabezas y así completar el juego.',
  'Cuando termines te saldrá un mensaje por tu triunfo.'
];
...
function chequearSiGano() {
  //COMPLETAR
  let contador = 1;
  for (let filas = 0; filas < grilla.length; filas++) {
    const grillas = grilla[filas];
    for (let columnas = 0; columnas < grillas.length; columnas++) {
      const conjuntoGrillas = grillas[columnas];
      if (conjuntoGrillas === contador) {
        contador++;
      } 
      else
      {
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    //COMPLETAR
    let valorTemp = grilla[filaPos1][columnaPos1];
    grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
    grilla[filaPos2][columnaPos2] = valorTemp;
}
